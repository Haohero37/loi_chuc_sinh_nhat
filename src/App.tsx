...
const BIRTHDAY_DAY   = 7;    // Ngày 7

const GIFTS = [
  "Chúc em tuổi mới luôn rạng rỡ như ánh mặt trời! ☀️",
  "Mong mọi ước mơ của em đều trở thành hiện thực. ✨",
  "Chúc em luôn bình an và hạnh phúc bên anh. 💖",
  "Mãi là cô gái đáng yêu nhất của anh nhé! 🎂",
  "Chúc em luôn xinh đẹp và tự tin vào bản thân. 🌸",
  "Mong em gặt hái được nhiều thành công trong công việc. 💼",
  "Chúc em có những chuyến đi thật thú vị và ý nghĩa. ✈️",
  "Mong nụ cười luôn nở trên môi em mỗi ngày. 😊",
  "Chúc em luôn khỏe mạnh và tràn đầy năng lượng. 💪",
  "Mong tình yêu của chúng mình mãi bền lâu. 👩‍❤️‍👨",
  "Chúc em luôn được yêu thương và trân trọng. 💎",
  "Món quà lớn nhất của anh chính là em! 🎁"
];
...
const MESSAGES = [
  {
    type: 'intro',
    title: "Chào em, người thương của anh ❤️",
    content: "Anh có một điều nhỏ muốn dành tặng em. Hãy nhấn nút phía dưới để bắt đầu nhé!",
  ...
  },
  {
    type: 'text',
    title: "Em biết không?",
    content: "Mỗi ngày trôi qua, anh lại thấy mình may mắn hơn một chút vì có em bên cạnh.",
    button: "Tiếp tục nào",
  ...
  },
  {
    ...
    content: "Anh vẫn muốn là người nắm tay em, cùng em đi qua những ngày nắng gắt hay mưa rào.",
    button: "Còn nữa nè...",
    bg: "from-fuchsia-100 to-rose-200"
  },
  {
    type: 'audio_wishes',
    title: "Lời chúc từ người thương 🎧",
  ...
  },
  {
    type: 'final',
    title: "Anh yêu em! 💖",
    content: "Cảm ơn em đã xuất hiện trong cuộc đời anh. Mãi bên nhau em nhé!",
  ...
  const currentMessage = useMemo(() => {
    ...
    if (msg.type === 'birthday_intro') {
      ...
      if (birthdayStatus === "today") {
        msg.title = `Hôm nay sinh nhật em tuổi ${age}! 🎂`;
        msg.content = `Chúc mừng sinh nhật công chúa ${age} tuổi của anh! Hôm nay là ngày tuyệt vời nhất vì có em xuất hiện trên đời. 🎉`;
      } else if (birthdayStatus === "soon") {
        msg.title = `Sắp đến sinh nhật em rồi... ✨`;
        msg.content = `Anh đang đếm ngược từng phút từng giây để cùng em đón tuổi ${age} thật rực rỡ đấy!`;
      } else {
        msg.title = "Và đặc biệt hơn hết...";
        msg.content = `Em sắp bước sang tuổi ${age} rồi — anh luôn có những lời chúc bí mật dành riêng cho em!`;
      }
    }
    return msg;
    ...
    <div className={`h-[100dvh] w-full overflow-hidden bg-gradient-to-br ${currentMessage.bg} transition-colors duration-1000 flex flex-col items-center justify-center p-4 md:p-6 relative`}>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <h1 className={`text-3xl md:text-4xl font-romantic font-bold mb-4 leading-tight ${
            isBirthday ? 'text-orange-700' : isPhotoRunner ? 'text-purple-700' : 'text-rose-700'
          }`}>
          ...
          {currentMessage.type === 'final' && (
            <div className="mb-4">
              ...
              {birthdayStatus === "today" ? (
                ...
                <div className="mt-2">
                  <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest mb-1">Đếm ngược đến sinh nhật em</p>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={nextStep}
            className={`group relative inline-flex items-center justify-center font-bold text-white transition-all duration-200 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 shadow-lg overflow-hidden ${
              currentMessage.type === 'final' ? 'px-6 py-3 text-sm' : 'px-8 py-4'
            } ${
      ...
      {/* Footer Decoration */}
      <div className={`absolute bottom-6 left-0 right-0 text-center font-romantic text-xl pointer-events-none ${
        isBirthday ? 'text-orange-400/60' : isPhotoRunner ? 'text-purple-400/60' : 'text-rose-400/60'
      }`}>
